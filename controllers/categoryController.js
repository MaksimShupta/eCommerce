...
const cat = CategoryModel(sequelize);
...
export const createCategory = async (req, res) => {
  try {
    ...
    const newCat = await cat.create({ name });
...
export const getCategoryById = async (req, res) => {
  try {
    const { id } = req.params;
    const searchedCat = await cat.findByPk(id);
    if (!searchedCat) {
      return res
        .status(404)
...
export const updateCategory = async (req, res) => {
  try {
    ...
    const { name } = req.body;
    const updatedCat = await cat.findByPk(id);
    if (!updatedCat) {
      return res
        .status(404)
        .json({ success: false, error: "Category not found" });
...
export const deleteCategory = async (req, res) => {
  try {
    const { id } = req.params;
    console.log("category id:", id);
    const findCat = await cat.findByPk(id);
